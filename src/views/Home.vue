...
export default {
  ...
  methods: {
    ...
    getStudentsInfoSucc(res) {
      ...
      if (res) {
        ...
        this.$store.commit("getStudentsData", {
          studentsData: this.studentData,
          schoolAverage: data.averageGrade
        });
      }
    },
    handleSearchClick() {
  ...
  watch: {
    classNum(newclassNum) {
      this.students = this.studentData["class" + newclassNum].slice(0, 9);
    }
  },
  created() {
